...
class Controller {
 public:
         ...
         bool is_Possible(int i, int j, bool is_playerone, bool is_con);  // Player가 선택한 위치에 돌을 놓을 수 있는지 확인
         void Reverse(int i, int j, int count, int dir, bool is_playerone);  // 돌을 뒤집는 함수
         bool is_Continue(bool is_playerone);  // Player가 돌을 놓을 수 있는곳이 있는 지확인
